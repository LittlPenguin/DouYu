...
| 帖子详情 | `_1` 延展 | 正文、评论、点赞、收藏、关注状态清楚；评论提交后短提示等待审核，列表不展示审核中字样 |
...
作品详情页评论输入采用 **B 方案延展版**，保留贴底轻量默认态和同一输入区展开语义，不采用 A 极简胶囊或 C 双阶段浮动按钮方案。折叠态只保留左侧展开按钮、单行输入占位和贴纸入口；展开态必须分层显示素材 Chip、多行输入、工具栏和右侧清空/发送操作，不能把头像、图片、@、#、贴纸、清空、发送和文本框挤在同一行。

- 默认态：贴底白色轻量胶囊，约 52-56dp 高，左侧头像或拼豆占位，中间单行提示“写下你的评论”，右侧表情入口；不展示发送按钮，不遮挡帖子正文、评论列表或系统导航。
- 展开态：输入区高度增加，支持多行输入；图片、@、#、表情和发送按钮保持同一操作区，不能变成遮挡正文的大面板。
- 缩略图与选择态：评论图片缩略图独立显示在输入区上方横向小卡片行，最多 9 张；已选 @ 用户、# 话题和贴纸使用轻量 Chip 展示，均不得挤压文字输入框。
- 颜色：外层 surface 使用白色，页面背景为 `#FFFBF7`；边框使用 `outline` 低透明度；发送按钮使用 `secondary` / `DoyuMint` 方向；图片、@、#、表情等辅助工具使用灰绿线性图标，避免高饱和粉紫和重渐变。
- 状态：发送按钮仅在有文字、图片或贴纸且未发送中时启用；禁用态降低透明度。@、#、表情必须打开真实用户搜索、话题搜索或内置贴纸面板，不能展示空入口或假成功。
