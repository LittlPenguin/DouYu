...
当前阶段的 Android 工作重心是 **第二轮 UI 统一基线**：先把 App Shell、通用组件、消息页和我的页统一成可复制样板，同时保留第一轮登录 + 社区样板作为契约和交互参考。在不新增真实 AI Provider、真实支付、增强审核、合规风控和完整玩家交易闭环的前提下，把已有开发态能力包装成可演示、可联调、边界清楚的 App。
...
- 第一轮已把社区发帖、评论、点赞、收藏补到 `CommunityRepository`，并接入社区样板页；第二轮需要把 App Shell、消息页和我的页统一成同一套状态与组件语言。
