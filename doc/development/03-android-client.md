...
- 支付最终状态以 `GET /payments/{paymentId}` 或订单详情服务端状态为准；客户端不能自动伪装真实支付成功。
- 支付页文案统一使用“联调支付单”“等待服务端确认”“服务端状态”，不得写“微信支付成功”“支付宝支付成功”作为当前事实。
...
- 支付页显式创建联调支付单，显示“等待联调回调 / 服务端确认”，不显示真实支付成功。
