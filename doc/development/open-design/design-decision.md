...
- 新增 Search 搜索页和上传帖子视觉参考，并为主页面搜索、底部上传、底部导航、消息列表和 Settings 列表补齐可点击去向；Search 仍无全局搜索后端，发帖使用现有 `POST /api/v1/posts`，成功后展示审核中。
...
- `post-compose-a.html`：上传帖子视觉参考；通过底部“上传”进入，包含选图、拍照、OSS 上传、失败重试、话题选择、发布和审核中状态。
