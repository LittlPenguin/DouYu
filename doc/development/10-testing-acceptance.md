...
- 当前只展示“联调支付单 / 等待服务端确认 / 服务端状态”，不得展示真实微信支付成功或支付宝支付成功。
...
- 有可用联调订单时，显式创建支付单并查询服务端状态；页面不显示真实支付成功。
...
- 当前真实后端种子数据 `/api/v1/products` 只有自营商品，玩家二手/定制商品详情真机路径未覆盖；本项暂按代码静态检查和后续数据补齐后复测处理。
