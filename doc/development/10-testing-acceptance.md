...
当前阶段的测试目标是支撑 **第六轮 UI/品牌与主链路展示收敛**：第三轮商城 UI/API 主体验收已关闭，第四轮登录态持久化和真实图文 seed、第五轮 Aliyun OSS Provider 骨架已作为既有基线保留；本轮重点验证主链路展示、好友/私信边界、作品详情评论互动、互动状态回显和个人互动资产页。不可用能力不能以空点击、假成功或误导性文案暴露给用户。
...
- 发帖提交后展示“审核中”；评论提交后只展示“评论已提交，等待审核”短提示，评论列表不展示“审核中”字样，不假装立即公开。
...
- App Shell 视觉一致：顶部栏、底部导航、卡片、按钮、Chip、状态页统一。
- 最新设计稿要求五个主 Tab 顶部统一为“新增 icon - 页面标题 - 搜索 icon”，新增菜单包含 Setting、AI 创作、上传帖子；底部导航必须有品牌化 Logo / icon，不得只显示文字、空框或 emoji。
- 消息页对齐 `13-ui-screen-blueprints.md` 和 `diagrams/message-profile-wireframes.svg`：通知/私信 Tab、列表密度、未读状态、空/错/未登录状态；历史 Stitch `_4` 不作为当前可用验收资产。
- 我的页对齐 `13-ui-screen-blueprints.md` 和 `diagrams/message-profile-wireframes.svg`，必须中文化并降低卡片嵌套；历史 Stitch `_3` 不作为当前可用验收资产。
...
- 对齐 `13-ui-screen-blueprints.md` 和 `diagrams/commerce-home-wireframe.svg` 的页面骨架：搜索栏、分类 Chip、Banner、双列商品卡；历史 Stitch `_2` 不作为当前可用验收资产。
...
- 文档不把已删除的 Stitch 截图写成当前可用验收资产。
- Stitch 目录只作为历史视觉参考和设计探索归档，不作为权威规范；当前可用设计入口为 `doc/development/diagrams/`。
...
- 社区提交：发帖提交后展示“审核中”；评论提交后只展示等待审核短提示，评论列表不展示“审核中”字样，不假装立即公开。
...
第六轮验收关闭主链路展示收敛、作品详情图片评论、@ 用户、# 话题、内置贴纸、点赞/收藏/关注状态回显和我的页四个互动资产页 MVP；不关闭视频评论、图片私信、用户自定义贴纸、付费表情包、话题运营后台、复杂推荐、生产级图片审核、真实 AI Provider、真实支付或地址管理。

设计稿验收补充：

- Open Design / `doc/development/open-design/` 必须包含主页面统一顶部栏、底部 Logo 导航、社区瀑布流、作品详情图片优先结构、评论浮动栏、多图堆叠/轮播、Search 搜索页、上传帖子 UI-only 页面、消息首页、私信对话详情、通知详情、我的页三资产 Tab、编辑资料页、Settings 多个内页和未来能力 UI-only 页面。
- 页面设计稿必须有基本可点击交互：搜索 icon 进入 `search-a.html`，新增菜单进入 Setting / AI / 上传帖子，底部导航五 Tab 互跳，私信行进入对话详情，通知行进入通知详情，Settings 行进入对应内页。
- Search 搜索页必须覆盖默认推荐、输入中、结果、空结果、加载失败、未登录受限和开发态，不得写成真实全站搜索 API 已完成。
- 上传帖子设计稿必须覆盖图片、正文、话题、上传中、上传失败、达到 9 图、未登录和审核中状态；不声明 Android / 后端发帖实现已完成。
- 社区首页设计稿采用双列瀑布流，图片视觉高度有 120dp-260dp 上限；不得把长图设计成无限撑高。
- 作品详情设计稿必须按“图片 -> 内容 -> 评论区域”排列；评论区必须包含标题、数量、文字评论、含图片评论、含 @/# 评论行、空态和加载失败状态。
- 评论工具条设计稿采用 Google 风格底部悬浮栏；展开态必须是图片/附件预览在上、评论内容在下、工具栏在底部；图片、@、# 均为 icon 工具；删除“帖子”工具，`#` 后不得保留旧发帖入口或含义不清的额外 icon；覆盖无图、一图、多图堆叠、上传中、上传失败、轮播预览、9 图上限和发送禁用态。
- 轮播预览必须清楚表达 gallery / carousel：大图、`2/5` 页码、左右箭头或滑动提示、底部缩略图 strip 和当前选中边框齐全。
- @ 用户和 # 话题设计必须覆盖搜索输入、结果、已选 Chip、空结果、加载失败或开发态边界；未接真实面板时不能空点击。
- 消息设计稿中，私信进入对话详情，通知进入通知详情；通知页不得出现会话输入状态。
- 我的页最新设计稿中，统计项为“获赞 / 作品 / 关注 / 粉丝”，不展示“我的订单 / 评论作品 / 关注作品”；“我的图纸 / 点赞作品 / 收藏作品”在同一 Tab 区展示内容卡；“编辑资料”进入 `profile-edit-a.html`。该设计目标不等于当前 Android 历史入口已删除或编辑资料路由已完成。
- Settings 设计稿必须覆盖 Settings 首页、账号与安全、隐私与权限、通知设置、关于与合规；合规相关入口必须标注待补或生产前必须补齐。
- 未来地图 API、真实支付 API、大模型生图 API 设计稿必须显著标注 UI-only / 当前不接后端 / 当前不可用；不得写成接口已接入。
...
- 默认态为贴底白色轻量输入条，左侧头像/拼豆占位、中间“写下你的评论”、右侧表情入口，不遮挡帖子正文、评论列表或系统导航。
- 点击左侧按钮、输入或图片按钮后，评论栏展开为更高的多行输入区；图片、@、#、输入框、表情和发送按钮均有真实动作，不允许空入口。
- 展开态文本框失去焦点、清空、发送成功或点击详情内容区后必须回到折叠态；点击图片、@、#、表情、清空、发送等内部工具时，不能因焦点变化提前折叠并吞掉本次操作。
- 发送按钮使用 Mint/Secondary 方向，图片/@/#/表情使用灰绿线性图标；不得回退到高饱和粉紫或 C 方案浮动按钮。
- 缩略图单独在工具条上方横向排列，不挤压文字输入框。
- 支持纯文字、纯图片、纯贴纸和图文贴纸混合评论；文字、图片、贴纸三者至少一种存在。
- @ 用户打开用户搜索弹层，# 话题打开话题搜索弹层，表情打开内置贴纸面板；选择后插入评论并随请求提交。
...
- 评论提交后只显示“评论已提交，等待审核”短提示；评论列表不展示“审核中”字样，也不假装立即公开。
- 评论列表渲染 `mediaAssets.publicUrl`、`mentions`、`topics` 和 `stickers`；图片加载失败时显示拼豆占位，不出现空白卡片。
- 帖子详情进入后使用 `likedByMe`、`favoritedByMe`、`followedAuthorByMe` 初始化点赞、收藏和关注按钮高亮；点赞、收藏、关注操作后重新拉取详情刷新状态和计数。
- 作品详情顶部内容使用 `Scaffold` padding 后贴合 `DoyuTopBar` 下方安全间距；`DoyuTopBar` 外层背景必须覆盖 status bar / 安全区，不再出现透明中空区域或露出作品头图。
- 历史 Android 互动资产验收仍记录为“点赞作品”“评论作品”“收藏作品”“关注作品”四个独立入口；但最新 UI 设计稿要求我的页首屏只展示“我的图纸 / 点赞作品 / 收藏作品”同组 Tab。实现阶段若采用最新设计，应同步调整验收口径，不能把设计稿和代码事实混写。

后端契约验收：

- 文本评论、纯图评论、纯贴纸评论、图文贴纸混合评论均可创建。
- 空文字 + 空图片 + 空贴纸返回参数错误。
- 仅有 @ 用户或 # 话题、没有文字/图片/贴纸时返回参数错误。
- 超过 9 张图片返回参数错误。
- 非当前用户文件、非 `POST_IMAGE` 文件、非 `image/*` 文件不能用于评论。
- 非法 `mentionUserIds`、`topicIds`、`stickerIds` 返回参数错误。
- 评论列表和发布评论响应均返回 `mediaFileIds`、`mediaAssets`、`mentions`、`topics`、`stickers`。
- 评论 @ 用户后写入 `MENTION` 通知。
- `GET /api/v1/users/search`、`/topics`、`/topics/{topicId}/posts`、`/sticker-packs` 可返回分页数据。
- `GET /api/v1/users/me/liked-posts`、`commented-posts`、`favorite-posts`、`followed-posts` 返回对应个人互动作品分页。
- 帖子详情登录态字段：已点赞、已收藏、已关注作者返回 `true`；未登录请求返回 `false`。

Android 单元验收：

- `ApiInterfaceContractTest` 覆盖新增路径、DTO 解析和 `CreateCommentRequest` 序列化。
- `CommunityRepositoryContractTest` 覆盖用户搜索、话题、贴纸包、话题作品和四个个人互动作品 Repository 方法。
...
- 截图保存到 `.qa-output/`，不提交。
- 验证作品详情顶部无透明空白，评论栏折叠态不堆叠、左侧按钮可展开/收起，展开态素材 Chip、多行输入、工具栏和发送区分层清楚；继续验证图片、@、#、贴纸、提交成功提示、评论列表无“审核中”、互动按钮高亮回显。若仍验历史代码入口，继续验我的页四个互动资产页跳转；若改按最新设计落地，则验“我的图纸 / 点赞作品 / 收藏作品”同组 Tab。

### 2026-06-04 未验通项

以下项目属于第六轮继续验收对象。在没有新的真机截图、日志、接口响应或测试输出前，不能写成“已通过”：

- 评论图片链路未验通：Photo Picker 选图、最多 9 张图片、`/uploads/presign -> PUT -> /uploads/confirm`、`POST_IMAGE` usage、上传失败时“整条评论不能提交、缩略图保留、提示重试”均缺少完整真机证据。
- 评论列表富内容渲染未验通：新评论进入审核态后不会立即公开，因此尚未在列表里验证图片、@ 用户、# 话题、贴纸渲染；评论图片加载失败时的拼豆占位也未完成真机验证。
- 纯贴纸评论未单独验通：已验证过 `@ + # + 贴纸` 组合评论提交，但“只有贴纸、无文字/图片”的评论还没有单独提交证据。
- 点赞状态回显未通过：上一轮真机操作后截图里点赞计数仍为 0，按钮也未高亮；需要单独复测点赞接口、详情页 `likedByMe` 初始化、计数刷新和点击定位。
- 收藏/关注持久回显未完整验通：当前页操作后已看到关注变为“已关注”、收藏高亮且计数变为 1，但还缺“退出详情后重新进入仍然高亮”的复验。
- 我的页历史四个互动资产页未完整验通：目前只可靠进入“点赞作品”并看到帖子卡；“评论作品 / 收藏作品 / 关注作品”独立页、列表卡跳转作品详情仍需稳定验证。最新 UI 设计稿已不再把“评论作品 / 关注作品”作为我的页首屏设计入口，后续实现需先明确采用历史入口回归还是最新三资产 Tab 口径。
- 真机图片环境仍需回归：如果 Feed/API 返回 `http://localhost:8081/...`，真机会把 localhost 当作手机自身而显示拼豆占位；必须确认 seed/public URL 使用 `.env` 中真机可访问的 LAN 地址，例如 `http://10.64.241.153:8081`，并确认后端实际返回一致。
- 后端完整测试需重新跑：上一轮只跑过 Android `:app:testDebugUnitTest` 和 `:app:assembleDebug` 时，若没有同步运行 `doyu-server mvn test`，后端契约不能写成当前轮已通过。
- ADB 坐标点按不稳定：后续真机验收优先使用 UIAutomator 文本定位、人工点按配合截图，或给关键 Compose 节点补测试 tag 后跑可控 UI 测试；不要用误入系统设置页的坐标脚本作为通过证据。

已明确通过的当前证据只限于：详情顶部无透明空白、评论栏默认/展开态、文本评论提交提示、@ 用户弹层和 Chip、# 话题弹层和 Chip、贴纸面板和 Chip、含 @/#/贴纸评论提交、关注/收藏当前页回显、我的页四入口可见、点赞作品页可进入。
