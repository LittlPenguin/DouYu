...
当前阶段的测试目标是支撑 **第二轮 UI 统一基线**：先确保第一轮登录 + 社区样板链路继续稳定，再把 App Shell、通用组件、消息页、我的页的 UI 口径和文档验收收紧；不可用能力不能以空点击、假成功或误导性文案暴露给用户。

当前已有后端契约测试、图纸算法测试、AI Stub Provider 测试和 Android 单元测试。本文档区分：

- 第一轮登录 + 社区基线验收。
- 第二轮 UI 统一基线验收。
- 后续阶段才需要补齐的 AI、商城、支付、审核、合规和风控验收。
...
| 截图视觉 QA | 阻塞：无在线设备 | 已定位 ADB：`D:\AndroidChace\platform-tools\adb.exe`；`adb devices` 当前无在线模拟器/真机，因此未生成截图，不能写成通过。 |
