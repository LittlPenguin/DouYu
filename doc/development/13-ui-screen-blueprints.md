...
- 底部导航：社区、商城、AI、消息、我的，使用品牌化 Logo / icon。
...
- 编辑资料为 UI 目标；后端资料更新接口存在，Android 需后续接入。
...
- Settings 子页不代表当前 Android 路由已完成。
...
| 起点 | 点击 | 目标 | 当前状态 |
|---|---|---|---|
| 主 Tab 顶部搜索 | 搜索 icon | `search-a.html` / 后续 Search 路由 | UI-only |
| 主 Tab 顶部新增 | Settings | `settings-home-a.html` / 当前 `settings` | 部分已有 |
...
| 消息通知 | 通知 | `notification-detail-a.html` | UI-only |
| 我的 | 编辑资料 | `profile-edit-a.html` | UI-only / 后端有 PATCH |
