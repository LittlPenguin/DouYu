...
| 社区 | Feed、帖子详情、评论列表、发帖、评论、点赞/取消、收藏/取消后端接口存在；Android 样板页已接 Repository 写操作；作品详情已接横向展开评论工具条和图片评论 | 视频评论、表情包、@ 用户、图片私信和生产级图片审核未完成；完整 MVVM 拆分和全页面一致性仍待做 | 第六轮只关闭纯字/纯图/图文评论展示闭环；评论图片使用 `POST_IMAGE` 上传和 `mediaAssets` 渲染，失败时保留缩略图并阻断提交 |
...
- 作品详情评论已支持横向展开工具条、最多 9 张评论图片、纯文字/纯图片/图文混合评论、上传失败阻断和评论列表图片渲染；后端契约返回 `mediaFileIds` 与 `mediaAssets`。
...
- 社区：作品详情评论工具条保持底部横向展开、平滑动画、最多 9 张图、纯字/纯图/图文混合；图片上传失败必须阻断整条评论提交并保留缩略图。
