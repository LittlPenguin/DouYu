...
| 社区 | Feed、帖子详情、评论列表、发帖、评论、点赞/取消、收藏/取消后端接口存在；Android 样板页已接 Repository 写操作；作品详情已按 B 方案接入横向展开评论工具条和图片评论 | 视频评论、表情包、@ 用户、图片私信和生产级图片审核未完成；完整 MVVM 拆分和全页面一致性仍待做；@、#、表情本轮只是禁用的视觉工具入口 | 第六轮只关闭纯字/纯图/图文评论展示闭环；评论图片使用 `POST_IMAGE` 上传和 `mediaAssets` 渲染，失败时保留缩略图并阻断提交 |
...
- 作品详情评论已按 **B 方案** 收敛：默认态为贴底轻量输入条，展开态为头像、图片、@、#、单行输入、表情和 Mint 发送按钮组成的同一条横向工具栏；缩略图独立在工具条上方横排展示，不挤压输入框。评论继续支持最多 9 张图片、纯文字/纯图片/图文混合、上传失败阻断和评论列表图片渲染；后端契约返回 `mediaFileIds` 与 `mediaAssets`。
...
- 社区：作品详情评论工具条保持 B 方案视觉口径：贴底轻量输入条、同一横向工具栏、Mint 发送按钮、灰绿线性工具图标和独立缩略图行；继续支持最多 9 张图、纯字/纯图/图文混合；图片上传失败必须阻断整条评论提交并保留缩略图。
- 后端：评论请求校验文字和图片不能同时为空、最多 9 张、文件必须属于当前用户且为 `POST_IMAGE` 图片；评论列表和发布响应都返回 `mediaAssets`。
- 文档 / QA：`10-testing-acceptance.md` 记录第六轮评论图片验收；真机只使用 `10.64.241.158:<当前端口>`。
