...
豆屿 Doyu 当前进入 **第三轮商城 UI/API 收敛阶段**。

第一轮已经把 **文档契约 + 登录/社区样板链路** 稳住：登录 + 社区接口以 `doc/development/05-api-contract.md` 为唯一契约源，社区 Feed、详情、发布、评论、点赞和收藏已作为可复制样板推进。第二轮已经把同一套 UI 方法推广到 App Shell、通用组件、消息页和我的页，形成 UI 统一基线。第三轮聚焦商城首页/商品列表、商品详情、购物车、订单确认、订单列表和支付单状态，把商城从“骨架可见”收敛为“可联调、边界清楚、不误导支付”的 MVP 链路。

本阶段默认策略：

- 采用 **MVP 收敛**，不是全入口保留，也不是重做全部信息架构。
- 第三轮只聚焦商城 UI/API 收敛：搜索、分类、Banner、双列商品卡、自营加购、玩家商品禁用标准购物车、订单地址缺口和联调支付状态。
...
|---|---|---|---|
| App Shell | 5 个主 Tab：社区、商城、AI 创作、消息、我的；NavHost 和底部导航已存在；第二轮已统一 App Shell、通用组件、消息页和我的页基线 | 仍需在后续模块扩散时回归顶部栏、底部栏、Search、状态页和响应式表现 | 第三轮商城收敛必须复用第二轮 `DoyuPage`、`DoyuTopBar`、底部 5 Tab、卡片、按钮、Chip、搜索、加载/空/错/未登录状态，不另起视觉口径 |
...
| 商城 | 商品列表/详情、购物车、订单、支付单和支付查询骨架存在；已有 `/products`、`/cart`、`/orders`、`/payments` 联调接口 | 地址管理未完成；真实微信/支付宝 SDK 未接；玩家二手/定制不支持标准购物车；订单确认如果继续伪装默认地址会误导；支付页如果自动展示成功会误导为真实支付 | 第三轮按 Stitch `_2` 收敛搜索/分类/Banner/双列卡；自营商品可加购；玩家商品、定制、二手禁用标准购物车并引导后续私信/咨询；地址缺口明确展示并禁止伪下单；支付状态页显式创建联调支付单，只展示服务端确认状态 |
...
- 订单确认仍受地址管理缺口限制；在地址接口未闭环前，客户端不能创建伪地址、不能伪装已具备真实收货履约能力。
...
- 不新增后端公共 API；商城第三轮优先接入已有 `/products`、`/cart`、`/orders`、`/payments` 能力。

## 下一步优先级

P0：第三轮商城 UI/API 收敛验收

- 商城首页/商品列表对齐 Stitch `_2`：搜索栏、分类 Chip、Banner、双列商品卡、价格/库存/销量层级清楚。
- 商品详情区分自营商品与玩家二手/定制；只有自营商品可进入标准加购，玩家商品禁用加购并展示后续咨询/直连边界。
- 购物车覆盖未登录、空车、加载失败、库存不足、数量修改、删除和结算入口；玩家商品不得混入标准购物车。
- 购物车联调契约已按真实后端收敛：写接口返回变更结果，Android 写后重新拉取 `GET /cart`；购物车项里的 `product` 是摘要对象，不按完整商品解析。
- 订单确认明确地址管理缺口；没有真实地址时禁用创建订单，不伪装“默认地址”、不创建订单或支付单。
- 支付状态页显式创建联调支付单，展示 paymentId、channel、amount、status 和服务端确认文案，不自动伪装真实支付成功。
- 在 360dp、390dp、430dp 下复查商城列表、详情、购物车、订单确认和支付状态页不挤压、不重叠。
