...
- 真机优先 QA：商城首页/商品列表对齐 Stitch `_2`：搜索栏、分类 Chip、Banner、双列商品卡、价格/库存/销量层级清楚。
- 真机优先 QA：商品详情区分自营商品与玩家二手/定制；只有自营商品可进入标准加购，玩家商品禁用加购并展示后续咨询/直连边界。
- 真机优先 QA：购物车覆盖未登录、空车、加载失败、库存不足、数量修改、删除和结算入口；玩家商品不得混入标准购物车。
